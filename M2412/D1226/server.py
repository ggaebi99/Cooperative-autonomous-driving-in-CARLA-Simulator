...
class VehicleSocket(threading.Thread, ):
                    ...
                    if data["PROTOCOL"] == "탈출":
                        ...
                        for vehicle in vehicles:
                            if vehicle.vehicle_id == self.rear and self.vehicle_id != vehicle.vehicle_id:
                                data = self.json_formatting(data["X_OR_THROTTLE"], data["Y_OR_STEER"], data["SPEED_OR_BRAKE"], self.frame, vehicle.front, vehicle.vehicle_id, vehicle.rear, data["PROTOCOL"], data["MODE"])
    ...
    def leader_vehicle_waypoint_sending_func(self, frame):
        while frame < len(waypoints) and self.flag:
            ...
            x, y, s = xys[0]
            data = self.json_formatting(x,y,s, frame, self.front, self.vehicle_id, self.rear, "SEND_DATA", True)
    ...
    def json_formatting(self, x_or_throttle, y_or_steer, speed_or_brake, frame, front, vehicle_id, rear, protocol, mode = False):
        data = {"X_OR_THROTTLE" : x_or_throttle, 
                "Y_OR_STEER" : y_or_steer, 
...
def temp_json_formatting(x_or_throttle, y_or_steer, speed_or_brake, frame, front, vehicle_id, rear, protocol, mode = False):
    data = {"X_OR_THROTTLE" : x_or_throttle, 
            "Y_OR_STEER" : y_or_steer, 
            "SPEED_OR_BRAKE" : speed_or_brake, 
            "FRAME" : frame,
