...
class KeyboardControl(object):
    ...
    def a(self, player):
        while True:
            current_x = player.get_transform().location.x 
            current_y = player.get_transform().location.y
            
            v = player.get_velocity()
            speed = math.sqrt(v.x**2 + v.y**2 + v.z**2)

            if self.last_text != f"{current_x}, {current_y}, {speed}\n" and math.sqrt((current_x - self.last_send[0]) ** 2 + (current_y - self.last_send[1]) ** 2) > 0.5:
                self.last_text = f"{current_x}, {current_y}, {speed}\n"
                self.last_send = [current_x, current_y]
            ...
            if event.type == pygame.QUIT:
                ...
                if isinstance(self._control, carla.VehicleControl):
                    if event.key == K_q:
                        ...
                        world.player.set_autopilot(self._autopilot_enabled)
                        thread = threading.Thread(target=self.a, args=(world.player,), daemon=True)
                        thread.start()
